...
const Toggle = ({
  ...
  <button
    ...
    className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none disabled:cursor-not-allowed disabled:opacity-60 ${
...
const AgentRow = ({ agent, permissions, onToggle, saving }) => {
...
export default function AgentPermissionsManager({
  backHref,
  backLabel = "Back",
  embedded = false,
}) {
      ...
      const permResults = await Promise.all(
        agentList.map(async (agent) => {
          try {
            ...
            console.error(
              "[AgentPermissions] perm fetch error for agent:",
              agent.id,
              err.code,
              err.message
            );
  ...
  const handleToggle = async (id, key, value) => {
    const previous = permissionsMap[id]?.[key] === true;
    ...
    try {
      ...
      console.error("[AgentPermissions] auto-save error:", err);
      setPermissionsMap((prev) => ({
        ...prev,
        [id]: { ...prev[id], [key]: previous },
      }));
  ...
  return (
    <div className={embedded ? "" : "min-h-screen bg-slate-50 p-6 lg:p-10"}>
      <div className={embedded ? "space-y-6" : "max-w-4xl mx-auto space-y-6"}>
        {backHref && (
          <div>
            <Button asChild variant="ghost" size="sm" className="px-0 text-slate-600 hover:bg-transparent hover:text-slate-900">
              <Link href={backHref}>
