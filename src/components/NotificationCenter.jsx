...
async function fetchDueFollowUps(userId) {
  ...
  const now = new Date();
  // Include follow-ups due within next 2 hours as well as all overdue ones
  ...
  await Promise.all(
    leadsSnap.docs.map(async (leadDoc) => {
      ...
      // Resolve mobile: check multiple field names, then fall back to customer record
...
export default function NotificationCenter({ userId }) {
  ...
  // One-time fetch of due/overdue follow-ups on mount
  useEffect(() => {
    if (!userId) return;
  ...
  const handleWhatsApp = (fu) => {
    const url = buildWhatsAppUrl(fu.leadMobile, fu.leadName, fu.notes);
      ...
      {open && (
        <div className="absolute right-0 top-11 z-50 w-96 rounded-2xl border border-slate-200 bg-white shadow-xl">
          ...
          <div className="max-h-[32rem] overflow-y-auto">
            ...
            {/* ── Firestore notifications section ── */}
