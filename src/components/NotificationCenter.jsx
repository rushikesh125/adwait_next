...
export default function NotificationCenter({ userId }) {
  ...
  const innerContent = (
    <>
      {/* Sticky header */}
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3 shrink-0">
      ...
      {permissionState === "denied" && (
        <div className="mx-3 mt-3 shrink-0 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
          <p className="text-[11px] text-amber-700">
            🔕 Notifications are blocked. Enable in browser settings to get background alerts.
          </p>
        </div>
      )}
          ...
          <section>
            ...
            {followUps.map((fu) => {
              ...
              return (
                <div
                  key={`fu-${fu.id}`}
                  className="border-b border-slate-100 px-4 py-3 bg-white hover:bg-amber-50/30 transition-colors"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-slate-800 truncate">{fu.leadName}</p>
                      <p className="mt-0.5 text-xs text-slate-500">
                        {fu.mode} follow-up ·{" "}
                        <span className={fu.isOverdue ? "text-red-600 font-semibold" : "text-amber-600 font-medium"}>
                      ...
                      <div className="mt-2 flex items-center gap-2 flex-wrap">
                        <button
                          onClick={() => { setOpen(false); router.push(`/agent-panel/leads/${fu.leadId}`); }}
      ...
      {open && (
        <>
          {/* ── MOBILE: floating panel via portal ──────────────────────────────
              Sits below the navbar (top-16 = 64px), floats over page content.
              Does NOT fill full width — matches image 2 appearance.
              Max height keeps it from growing too tall; content scrolls inside.
          ────────────────────────────────────────────────────────────────────── */}
          {mounted && createPortal(
            <div className="sm:hidden">
              ...
              />
              {/* Floating panel — NOT full width, aligned to right side */}
              <div
                id="notif-mobile-panel"
                className="
    fixed  right-2
    top-[72px] z-[999]
  flex flex-col overflow-hidden  rounded-2xl  border border-slate-200 bg-white   shadow-2xl    ring-1  ring-black/5   animate-in  fade-in slide-in-from-top-2 duration-200
  "
                style={{
