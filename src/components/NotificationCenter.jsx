...
export default function NotificationCenter({ userId }) {
  ...
  function resolveNotificationLink(n) {
    ...
    if (n.type === "quotation_accepted" || n.type === "quotation_rejected") {
      if (n.quotationId) {
        return `/agent-panel/my-quotation?quoteId=${n.quotationId}`;
      }

      return "/agent-panel/my-quotation";
    }

    // follow-up reminders
    if (n.type === "follow_up_reminder") {
      const leadId = n.metadata?.leadId || n.leadId;

      if (leadId) {
        return `/agent-panel/leads/${leadId}`;
      }

      return "/agent-panel/leads";
    }

    // installment / vendor payment reminders
    if (n.type === "vendor_payment_due") {
      const bookingId = n.metadata?.bookingId || n.bookingId;

      if (bookingId) {
        return `/agent-panel/bookings/${bookingId}`;
      }

      return "/agent-panel/bookings";
    }
    ...
    if (n.type === "invoice_generated") {
      const invoiceId = n.metadata?.invoiceId || n.invoiceId;

      if (invoiceId) {
        return `/agent-panel/invoices/${invoiceId}`;
      }

      return "/agent-panel/invoices";
    }
              ...
              {permissionState === "denied" && (
                <div className="mx-3 mt-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
                  <p className="text-[11px] text-amber-700">
                    🔕 Notifications are blocked. Enable in browser settings to
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            >
                              <ModeIcon
                                className={`h-4 w-4 ${overdue ? "text-red-600" : "text-rose-500"}`}
                              />
                            </div>
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-2 flex items-center gap-2">
                                <button
                                  onClick={() => {
                                    const leadId =
                                      fu.metadata?.leadId ?? fu.leadId;
                                    setOpen(false);
                                    if (fu.link) router.push(fu.link);
                                    else if (leadId)
                                      router.push(
                                        `/agent-panel/leads/${leadId}`,
                                      );
                                  }}
                                  className="text-[11px] font-semibold text-theme-primary hover:underline"
                                >
