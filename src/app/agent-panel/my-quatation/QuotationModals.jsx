    ...
    <Card className="border-dashed border-2 border-theme-primary/40 bg-theme-muted/10">
      ...
      <CardContent className="p-3 sm:p-4 pt-0 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          {!plansWithPrice.includes(selectedMealPlan) &&
...
const CustomActivityForm = ({ state, onAdd, onCancel }) => {
  ...
  const handleSubmit = () => {
    ...
    const totalPrice =
      (parseFloat(form.pricePerPerson) || 0) *
      (parseInt(form.participants) || 1);
    onAdd({
      ...form,
      totalPrice,
      fitRatePerPerson: form.pricePerPerson,
      groupRatePerPerson: form.pricePerPerson,
    });
  };
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e) =>
                set("participants", parseInt(e.target.value) || 1)
              }
              className="text-sm"
            />
          </div>
...
const QuotationModals = ({
  ...
  const noHotelsFound =
    SelectedDestination && hotelsForSelectedState.length === 0;
  const noActivitiesFound =
    SelectedDestination &&
    !isFetchingActivities &&
    activitiesForSelectedState.length === 0;
  ...
  const markupInputValue =
    editingQuotation?.markupValue ?? editingQuotation?.markup ?? 0;
                      ...
                      <div className="mt-3 sm:mt-4 grid grid-cols-2 gap-2 text-xs sm:text-sm">
                        ...
                        <div>
                          ...
                          <p
                            ...
                            title={hotel.selectedRoomCategory}
                          >
                        ...
                        <div>
                          ...
                          <p className="font-medium text-xs">
                            {hotel.numDouble || 0}D, {hotel.numExtraAdult || 0}
                            A, {hotel.numExtraChild || 0}C
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          ...
                          {viewingQuotation?.transportSummary?.ac
                            ? " (AC)"
                            : ""}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          ₹
                          {viewingQuotation?.transportSummary?.permitCharges ||
                            0}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span>
                          ₹
                          {viewingQuotation?.transportSummary?.otherCharges ||
                            0}
                        </span>
                      </div>
                          ...
                          {noHotelsFound ? (
                            <div className="flex items-center gap-2">
                              <p className="text-sm text-muted-foreground flex-1">
                                No hotels found for this state.
                              </p>
                              {!showCustomHotelForm && (
                                <Button
                                  size="sm"
                    ...
                    <>
                      ...
                      <div className="hidden lg:block rounded-md border overflow-x-auto">
                        <Table>
                          ...
                          <TableBody>
                            {editingQuotation.hotelSummary.map(
                              (hotel, index) => {
                                ...
                                return (
                                  <TableRow
                                    key={index}
                                    className={
                                      hotel.isCustom ? "bg-theme-muted/10" : ""
                                    }
                                  >
                      ...
                      <div className="lg:hidden space-y-4">
                        {editingQuotation.hotelSummary.map((hotel, index) => {
                          ...
                          return (
                            <Card
                              key={index}
                              className={`border-theme-muted ${hotel.isCustom ? "bg-theme-muted/10" : ""}`}
                            >
                          ...
                          {selectedTransportStateId && (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 pt-4 border-t">
                              <div>
                                ...
                                <p
                                  className="font-medium mt-1 text-sm truncate"
                                  title={
                                    editingQuotation?.transportSummary
                                      ?.packageName
                                  }
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div className="space-y-2">
                          ...
                          {noActivitiesFound ? (
                            <div className="flex items-center gap-2">
                              <p className="text-sm text-muted-foreground flex-1">
                                No activities found for this state.
                              </p>
                              {!showCustomActivityForm && (
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    setShowCustomActivityForm(true)
                                  }
                                  className="bg-theme-primary hover:bg-theme-secondary text-xs whitespace-nowrap"
                                >
                                  <PenLine className="h-3 w-3 mr-1" />
                            ...
                            {editingQuotation.activitySummary.map(
                              (activity, index) => (
                                <TableRow
                                  key={index}
                                  className={
                                    activity.isCustom ? "bg-theme-muted/10" : ""
                                  }
                                >
                                  <TableCell className="font-medium">
                                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="md:hidden space-y-3">
                        {editingQuotation.activitySummary.map(
                          (activity, index) => (
                            <Card
                              key={index}
                              className={`border-theme-muted ${activity.isCustom ? "bg-theme-muted/10" : ""}`}
                            >
                                ...
                                <div className="flex justify-between items-start gap-2 mb-3">
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      {activity.isCustom && (
                                        <span className="text-xs bg-theme-primary/10 text-theme-primary px-1 rounded">
                                          Custom
                                        </span>
                                      )}
                                    </div>
              ...
              <Card className="bg-theme-muted/30">
                ...
                <CardContent className="space-y-4 sm:space-y-6 p-3 sm:p-6 pt-0">
                  <div className="grid grid-cols-3 gap-3 text-sm">
                    <div className="text-center p-2 rounded-md bg-background border">
                      ...
                      <p className="font-semibold text-theme-primary">
                        ₹
                        {(
                          editingQuotation?.hotelSummary?.reduce(
                            (s, h) => s + (h.hotelTotal || 0),
                            0,
                          ) || 0
                        ).toFixed(0)}
                      </p>
                  ...
                  <div className="pt-3 border-t flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    {markupMode === "percentage" &&
                      (editingQuotation?.markupValue || 0) > 0 && (
                        <p className="text-xs text-muted-foreground">
                          Base ₹{baseTotal.toFixed(0)} +{" "}
                          {editingQuotation.markupValue}% markup
                        </p>
                      )}
                  </div>
                </CardContent>
      ...
      <Dialog open={showSaveAsModal} onOpenChange={setShowSaveAsModal}>
        <DialogContent className="w-[95vw] max-w-md p-4 sm:p-6">
          ...
          <div className="space-y-4 sm:space-y-6 py-4">
            <div className="space-y-2">
              <Label htmlFor="newPackageName" className="text-sm">
                New Package Name
              </Label>
              <Input
                id="newPackageName"
