...
export default function UnassignedLeadsPage() {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setLeads((prev) => prev.filter((l) => l.id !== assigningLead.id));
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Inbox className="h-6 w-6 text-amber-500" /> Unassigned Leads
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            {leads.length} lead{leads.length !== 1 ? "s" : ""} waiting for assignment
          </p>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <p className="text-sm text-slate-400 text-center py-12">
              {search ? "No leads match your search" : "No unassigned leads"}
            </p>
          ) : (
            <Table>
              <TableHeader>
                ...
                {filtered.map((lead) => (
                  <TableRow key={lead.id} className="hover:bg-slate-50/60">
                    ...
                    <TableCell>
                      <Button
                        size="sm"
                        className="rounded-xl bg-theme-primary hover:bg-theme-secondary text-white gap-1.5 text-xs h-8"
      ...
      <Dialog open={!!assigningLead} onOpenChange={(open) => { if (!open) setAssigningLead(null); }}>
        <DialogContent className="rounded-2xl max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Assign <strong>{assigningLead?.name}</strong> to an agent on your team.
            </DialogDescription>
          </DialogHeader>
          <div className="py-3">
