...
const PLANS = [
  ...
  },
  {
    key: "cp",
    label: "CP",
    full: "Continental Plan",
    color: "bg-violet-50 text-violet-700 border-violet-200",
  },
  {
    key: "map",
    label: "MAP",
    full: "Modified American Plan",
...
const inputCls =
  "h-9 border border-slate-200 rounded-lg px-3 bg-white focus:border-theme-primary focus:ring-2 focus:ring-theme-primary/20 outline-none text-sm text-slate-800 placeholder:text-slate-400 transition-all w-full";
  ...
  return (
    <div
      ...
      className={`relative border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200
        ${
          drag
            ? "border-theme-primary bg-theme-muted scale-[1.01]"
            : "border-slate-200 bg-slate-50/60 hover:border-theme-primary/50 hover:bg-theme-muted/40"
        }
    ...
    >
      ...
      <div
        className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 transition-all
      ...
      {!processing && (
        <div className="flex flex-wrap justify-center gap-2">
          {[
            ...
            <span
              key={t}
              className="text-xs px-3 py-1.5 rounded-full bg-white border border-slate-100 text-slate-500 font-medium shadow-sm"
            >
              ✓ {t}
            </span>
          ))}
  ...
  const stats = [
    {
      ...
      col: "text-theme-primary",
      bg: "bg-theme-muted/60",
    },
    {
      icon: BedDouble,
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {stats.map(({ icon: Icon, label, value, col, bg }) => (
        ...
        >
          <div className={`p-2 rounded-lg ${bg} ${col}`}>
...
// ─── Pricing Row for a single season ─────────────────────────────────────────
// pricing = { ep: 12000, cp: 14000, map: 16000, ap: 14000 }
function PricingGrid({
  ...
  if (!activePlans.length && !isEditing) {
    return (
      <p className="text-xs text-slate-300 italic py-1">No pricing data</p>
    );
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {activePlans.map((plan) => {
          ...
          return (
            <div
              key={plan.key}
              className={`rounded-xl border px-3 py-2.5 ${plan.color}`}
            >
              ...
              {isEditing ? (
                <div className="relative">
                  <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] font-semibold opacity-60">
                    ₹
                  </span>
                  <input
                    type="number"
                    ...
                    className="h-8 w-full border border-current/20 rounded-lg pl-5 pr-2 text-right text-sm font-bold bg-white/70 focus:bg-white outline-none focus:ring-1 focus:ring-current/30"
                  />
                </div>
              ) : (
                <p
                  className={`text-sm font-bold ${price === 0 ? "opacity-40" : ""}`}
                >
                  {price === 0
                    ? "—"
              ...
              <p className="text-[9px] opacity-50 mt-0.5 font-medium truncate">
                {plan.full}
              </p>
            </div>
          );
...
function SeasonCard({
  ...
  const conflicts = allSeasons.filter((s, i) => {
    ...
    return (
      rangesOverlap(season.start, season.end, s.start, s.end) &&
  ...
  return (
    <div
      className={`rounded-xl border overflow-hidden ${hasConflict ? "border-red-200 ring-1 ring-red-100" : "border-slate-150 border-slate-200"}`}
    >
      {/* Season header */}
      <div
        onClick={() => setOpen((o) => !o)}
      ...
      >
        ...
        {isEditing && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onRemove(hotelIndex, roomIndex, seasonIndex);
            }}
...
function RoomCard({
  ...
  return (
    <div
      className={`rounded-xl border ${hasConflict ? "border-red-200" : "border-slate-200"} overflow-hidden shadow-sm`}
    >
      <div
        onClick={() => setOpen((o) => !o)}
      ...
      >
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <Badge
            variant="outline"
            className="text-[10px] shrink-0 text-slate-500 border-slate-200"
          >
            {room.seasons?.length || 0} season
            {(room.seasons?.length || 0) !== 1 ? "s" : ""}
          </Badge>
          {hasConflict && (
            <Badge
              variant="destructive"
        ...
        <div className="p-3 space-y-2 bg-white">
          {!room.seasons || room.seasons.length === 0 ? (
            <p className="text-xs text-slate-400 text-center py-5">
              No seasons.{isEditing && " Click '+ Season' above to add one."}
            </p>
          ) : (
...
function HotelSection({
  ...
  const anyConflict = (hotel.rooms || []).some((r) =>
    roomHasConflict(r.seasons || []),
  );
  ...
  return (
    <Card
      className={`border ${anyConflict ? "border-red-200" : "border-slate-200"} shadow-sm overflow-hidden`}
    >
      {/* ── Hotel header bar ── */}
      <div
        className={`px-5 py-4 border-b ${anyConflict ? "border-red-100 bg-red-50/30" : "border-slate-100"} bg-gradient-to-r from-theme-muted/50 to-white`}
      >
            ...
            <div className="flex-1 min-w-0">
              {editing ? (
                <Input
                  ...
                  className="font-bold text-sm h-8 mb-2 max-w-xs"
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1 text-xs text-slate-500">
                  ...
                  {editing ? (
                    <div className="flex items-center gap-1">
                      <Input
                        value={hotel.city}
                        onChange={(e) => fld("city", e.target.value)}
                ...
                {editing ? (
                  <select
                    ...
                    onChange={(e) => fld("starRating", e.target.value)}
                  ...
                  <Badge
                    variant="outline"
                    className={`text-[10px] flex items-center gap-1 ${starColor}`}
                  >
                ...
                {editing ? (
                  <div className="flex items-center gap-1">
                    <span className="text-[10px] font-black text-blue-600 bg-blue-50 border border-blue-200 rounded px-1.5 py-0.5">
                      G
                    </span>
                    <Input
                      value={hotel.googleRating || ""}
                      onChange={(e) => fld("googleRating", e.target.value)}
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] border-blue-200 text-blue-700 bg-blue-50 gap-1"
                  >
              ...
              {/* Hotel link — edit mode */}
              {editing && (
                <div className="mt-2 flex items-center gap-2">
                  <ExternalLink className="h-3 w-3 text-slate-400 shrink-0" />
                  <Input
                    value={hotel.hotelLink || ""}
                    onChange={(e) => fld("hotelLink", e.target.value)}
          ...
          {anyConflict && (
            <Alert className="border-red-200 bg-red-50 py-2">
              ...
              <AlertDescription className="text-red-700 text-xs font-medium">
                Some room categories have overlapping season dates. Review and
...
export default function HotelUploadPage() {
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      toast.success(
        `${data.hotels.length} hotel${data.hotels.length !== 1 ? "s" : ""} extracted!`,
      );
    } catch {
  ...
  const updateHotelField = (hIdx, key, val) =>
    setHotels((prev) =>
      prev.map((h, i) => (i !== hIdx ? h : { ...h, [key]: val })),
    );

  // hotelIndex, roomIndex, seasonIndex, key, value
  // key === "__roomName" → room.categoryName
  // key === "pricing"   → season.pricing
  // else                → season[key]
  ...
  const handleRemoveRoom = (hIdx, rIdx) => {
    setHotels((prev) =>
      prev.map((h, hi) =>
        hi !== hIdx ? h : { ...h, rooms: h.rooms.filter((_, i) => i !== rIdx) },
      ),
    );
    ...
    setHotels((prev) =>
      prev.map((h, hi) =>
        hi !== hIdx
          ...
          : {
              ...h,
              rooms: [{ categoryName: "", seasons: [] }, ...h.rooms],
            },
      ),
    );
  };
        ...
        hi !== hIdx
          ...
          : {
              ...
              rooms: h.rooms.map((r, ri) =>
                ri !== rIdx
                  ? r
                  : { ...r, seasons: r.seasons.filter((_, i) => i !== sIdx) },
              ),
            },
      ),
    );
  ...
  const totalConflicts = hotels
    ? hotels.reduce(
        (a, h) =>
          a +
          (h.rooms || []).filter((r) => roomHasConflict(r.seasons || []))
            .length,
        0,
      )
    : 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-5xl mx-auto sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="font-bold text-slate-900 text-base">
                    Bulk Hotel Data Import
                  </h2>
                  <p className="text-sm text-slate-500 mt-1">
                    Upload your hotel inventory Excel file. All hotels, rooms,
                    seasons and meal-plan pricing will be extracted and
                    displayed for review before saving to Firestore.
                  </p>
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    n: "1",
                    title: "Upload",
                    desc: "Drop your .xlsx file",
                  },
                  {
                    icon: Eye,
                    n: "2",
                    title: "Review & Edit",
                    desc: "Fix any extracted data",
                  },
                  {
                    icon: ClipboardCheck,
                    n: "3",
                    title: "Save to DB",
                    desc: "Push confirmed data to Firestore",
                  },
                  ...
                  >
                    <div className="w-7 h-7 rounded-full bg-theme-primary/10 text-theme-primary text-xs font-black flex items-center justify-center shrink-0">
                      {n}
                    </div>
                    <div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
                <Sparkles className="h-3.5 w-3.5 text-theme-primary" /> Expected
                  ...
                  <thead>
                    <tr className="bg-slate-50">
                      {[
                        ...
                        <th
                          key={h}
                          className="px-2.5 py-2 text-left font-semibold text-slate-500 border-b border-slate-100 whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {[
                      ...
                      >
                        {row.map((cell, j) => (
                          <td
                            key={j}
                            className={`px-2.5 py-2 whitespace-nowrap ${!cell ? "text-slate-200" : "text-slate-700"}`}
                          >
                            {cell || "↑"}
                          </td>
                        ))}
              ...
              <p className="text-[11px] text-slate-400 mt-3">
                ↑ Hotel fields (State, City, Name, etc.) only need to be filled
                on the first row — they are carried forward automatically.
                Season fields repeat on every room row for that season.
              </p>
            </div>
          </div>
        )}
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    ...
                    on any hotel section to modify data inline.
                  </p>
                </div>
                <div>
                  {totalConflicts === 0 ? (
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-1.5 font-semibold">
                      <AlertTriangle className="h-3.5 w-3.5" /> {totalConflicts}{" "}
