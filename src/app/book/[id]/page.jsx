...
import { Train, Loader2, Lock, FileText, AlertCircle, CheckCircle, User } from "lucide-react";
...
export default function PublicBookingPage({ params: paramsPromise }) {
  ...
  const validate = () => {
    ...
    if (!/^\d{10}$/.test(passenger.mobile)) newErrors.mobile = "10-digit number required";
    if (!/^\S+@\S+\.\S+$/.test(passenger.email)) newErrors.email = "Invalid email";
  ...
  if (trip === "not-found") return <StatusMessage icon={<AlertCircle className="w-12 h-12 text-red-500" />} title="Not Found" description="This trip does not exist." />;
  if (trip.status === "draft") return <StatusMessage icon={<FileText className="w-12 h-12 text-slate-400" />} title="Draft Mode" description="This form is not public yet." />;
  if (trip.status === "closed") return <StatusMessage icon={<Lock className="w-12 h-12 text-amber-500" />} title="Closed" description="Bookings are no longer accepted." />;
  ...
  if (submitted) return (
    <StatusMessage 
      ...
      action={<Button className="bg-theme-primary" onClick={() => window.location.reload()}>Submit New</Button>} 
    />
  );
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className="border-t-8 border-t-theme-primary shadow-sm bg-white overflow-hidden">
          <CardHeader>
            ...
            <CardTitle className="text-3xl font-black text-slate-900">{trip?.tripName}</CardTitle>
        ...
        <form onSubmit={handleSubmit} autoComplete="off" className="space-y-6 pb-20">
          ...
          <FormSection title="Full Name (as per Aadhaar)" required error={errors.name}>
            <Input 
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormSection title="Age" required error={errors.age}>
              <Input type="number" placeholder="Years" className="theme-input font-bold" value={passenger.age} onChange={(e) => updateField('age', e.target.value)} />
            </FormSection>
            <FormSection title="Gender" required>
              <RadioGroup value={passenger.gender} onValueChange={(v) => updateField('gender', v)} className="flex h-10 items-center gap-4">
                {["Male", "Female"].map((g) => (
          ...
          <FormSection title="Seat Preference">
            <Select value={passenger.preference} onValueChange={(v) => updateField('preference', v)}>
          ...
          <FormSection title="Residential Address" required error={errors.address}>
            <Input placeholder="City, State" className="theme-input" value={passenger.address} onChange={(e) => updateField('address', e.target.value)} />
          </FormSection>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormSection title="Mobile Number" required error={errors.mobile}>
              <Input placeholder="10 digit number" className="theme-input" value={passenger.mobile} onChange={(e) => updateField('mobile', e.target.value)} />
            </FormSection>
            <FormSection title="Email Address" required error={errors.email}>
              <Input type="email" placeholder="personal@email.com" className="theme-input font-medium" value={passenger.email} onChange={(e) => updateField('email', e.target.value)} />
            </FormSection>
          </div>

          <Button type="submit" disabled={isSubmitting} className="w-full bg-theme-primary hover:bg-theme-dark text-white font-bold h-14 shadow-lg text-lg">
...
function FormSection({ title, required, children, error }) {
  return (
    <div className={`p-5 rounded-xl bg-white border border-slate-100 shadow-sm ${error ? "border-red-200 bg-red-50/10" : ""}`}>
      <Label className="text-[10px] font-black text-theme-primary uppercase tracking-widest block mb-2">
        {title} {required && <span className="text-red-500">*</span>}
      </Label>
      {children}
      {error && <p className="mt-2 text-[10px] font-bold text-red-500 uppercase italic">{error}</p>}
    </div>
  );
}