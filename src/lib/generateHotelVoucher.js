  ...
  export async function generateHotelVoucherPDF(voucher = {}) {
    ...
    // Hotel name — bold
      // Hotel name — bold
    // Hotel name + inline Maps link on same line
    setFont("bold", 11, BRAND.text);
    doc.text(voucher.hotelName || "\u2014", colL, y);

    if (voucher.googleMapsLink) {
      const hotelNameWidth = doc.getTextWidth(voucher.hotelName || "");
      
      // Small separator
      setFont("normal", 8, BRAND.text);
      doc.text(" (", colL + hotelNameWidth, y);
      const parenWidth = doc.getTextWidth(" (");

      // Clickable blue link text
      setFont("normal", 8, BRAND.accent);
      doc.textWithLink("View on Google Maps", colL + hotelNameWidth + parenWidth, y, {
        url: voucher.googleMapsLink,
      });
      const linkWidth = doc.getTextWidth("View on Google Maps");

      // Closing paren
      setFont("normal", 8, BRAND.text);
      doc.text(")", colL + hotelNameWidth + parenWidth + linkWidth, y);
    }
    ...
    if (voucher.address) {
      setFont("normal", 8, BRAND.muted);
      const addrLines = doc.splitTextToSize(`Adress : ${voucher.address}`, CW);
    ...
    if (voucher.phone) {
      setFont("normal", 8, BRAND.muted);
            doc.text(`Contact: ${voucher.phone}`, ML, y);
            y += 6;
          }
