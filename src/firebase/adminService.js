...
export const getLeadsByAdmin = async (adminId) => {
  if (!adminId) return [];
  const snap = await getDocs(
    query(collection(db, "leads"), where("adminId", "==", adminId))
  );
  return snap.docs
...
export const assignLeadToAgent = async (leadId, agent) => {
...
export const getQuotationsByAdmin = async (agentIds) => {
  ...
  const results = await Promise.all(
    agentIds.map((uid) =>
      getDocs(collection(db, "saved_packages_by_agents", uid, "packages")).then(
...
export const getAdminDashboardStats = async (adminId) => {
  ...
  const [agents, leads, bookings] = await Promise.all([
    getAgentsByAdmin(adminId),
    getLeadsByAdmin(adminId),
    getBookingsByAdmin(adminId),
  ]);

  const agentIds = agents.map((a) => a.id);
  const quotations = await getQuotationsByAdmin(agentIds);
